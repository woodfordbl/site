...
4. **One `.hover-reveal` utility** (opacity, `--duration-reveal`, `--ease-out-strong`, gated `@media (hover:hover)`, `focus-within` always-on) applied to: canvas gutter, sidebar row actions (currently instant snap), chevron/icon swap, media toolbar/handles, column dividers (currently 300ms delay vs media's 0ms — same affordance, opposite feel).
