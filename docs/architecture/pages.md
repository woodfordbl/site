...
The header menu's **Version history** action is presentation-aware. On fine pointers it opens a full-view overlay ([`PageVersionHistoryView`](../../src/components/pages/page-version-history-view.tsx)): a version list (descriptors newest-first via [`usePageSnapshots`](../../src/db/queries/use-page-snapshots.ts)) beside a **read-only preview** of the selected checkpoint. On touch (`useIsCoarsePrimaryPointer`) it is a nested **sub-drawer** of the page menu instead ([`PageVersionHistorySubmenu`](../../src/components/pages/page-version-history-submenu.tsx)): a `DropdownMenuSub` listing the versions (rows use `closeOnClick={false}`), each tap pushing a further [`MenuDrawerSubDrawer`](../../src/components/ui/menu-presentation.tsx) with the preview + restore. Both share the read-only preview ([`SnapshotPreview`](../../src/components/pages/snapshot-preview.tsx)) — the canvas renderer in view mode, [`CanvasBlocksReadOnly`](../../src/components/canvas/page-canvas-server.tsx) `mode="view"` (read-only `View` components, no `contentEditable`/gutters) wrapped in the snapshot's [`pageContentTypographyProps`](../../src/lib/pages/page-content-typography.ts), with the payload loaded on demand by [`usePageSnapshotContent`](../../src/db/queries/use-page-snapshot-content.ts). **Restore this version** (confirm dialog) calls [`restorePageSnapshot`](../../src/lib/pages/restore-page-snapshot.ts).

Each edit schedules a debounced capture ([`schedulePageSnapshotCapture`](../../src/lib/pages/capture-page-snapshot.ts)) that collapses a **10-minute wall-clock window** into one checkpoint of the page's full state (blocks + order + title + icon + settings); content-hash dedupe skips no-op edits. Older history coarsens over time ([`thinSnapshotDescriptors`](../../src/lib/pages/thin-page-snapshots.ts): last 1h every 10-min bucket, then hourly/daily/weekly, hard-capped at 40/page). `restorePageSnapshot` captures the current state first (so the revert is undoable), then re-applies blocks atomically via `applyPageBlockDiff` and the page metadata. Storage layout and retention: [local-first-persistence — Page snapshots](./local-first-persistence.md#page-snapshots-version-history). Snapshots are cleared on **Reset page**, hard delete, **Reset all**, and **Save to source**.
