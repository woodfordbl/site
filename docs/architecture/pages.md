...
| Drop a sidebar page onto an open canvas | Re-nest the dragged page under the open page and append a child `pageLink` at its end — the cross-surface counterpart of "drop on row (middle band)", guarded by [`canDropPageIntoCanvas`](../../src/lib/pages/page-canvas-drop.ts) |
