...
The picker, render, and triggers are wired through a small [`PageCoverProvider`](../../src/components/pages/page-cover-context.tsx) (one shared [`PageCoverDialog`](../../src/components/pages/page-cover-menu.tsx)); `usePageCover().openPicker` is called from the page-header photo button ([`page-header.tsx`](../../src/components/pages/page-header.tsx), which prefetches the Unsplash default feed on hover), the ⋯ menu item, and the on-cover **Change** button. The dialog has Unsplash / Link / Upload tabs; the Unsplash panel loads the popular editorial feed on open (empty query → `/api/unsplash/photos` feed) so default images show before typing. Unsplash search/download stay proxied through Nitro ([`/api/unsplash/search`](../../routes/api/unsplash/search.get.ts), [`/api/unsplash/download`](../../routes/api/unsplash/download.post.ts), registered in [`vite.config.ts`](../../vite.config.ts)) so `UNSPLASH_ACCESS_KEY` stays server-side; selecting a photo fires the required download ping and stores photographer `credit`.

[`PageCover`](../../src/components/pages/page-cover.tsx) renders inside the canvas scroll region as `coverSlot` (above `headerSlot`/`titleSlot`; full-bleed via negative margins **and** a `w-[calc(100%+padding)]` width in [`page-title-layout.ts`](../../src/lib/pages/page-title-layout.ts) so it spans under the overlay scrollbar). It shows a media-style hover toolbar ([`ButtonGroup variant="overlay"`]) — **Change / Reposition / Download / Copy / Copy link** (reusing [`media-actions.ts`](../../src/lib/media/media-actions.ts)) — plus a right-click menu (Copy image / Copy link to image). **Reposition** is interactive on the image itself: the cover is `object-cover` (fit-to-width) and a vertical pointer drag pans `focalY` (`object-position`), reusing the document-listener + rAF drag pattern from [`useMediaResize`](../../src/components/blocks/types/media/use-media-resize.ts). Photographer **attribution is shown only in the picker grid** (photos shown together), not on the rendered cover.
