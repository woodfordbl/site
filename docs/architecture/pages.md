...
`pageLink` blocks store `props.pageId` and an optional `props.variant`: `child` (slash **New Page** subpage on the current canvas) or `linked` (slash **Link To Page** reference to an existing page). Label text is resolved at render time from the merged page list via `usePageSummary`. [`pageLinkShowsExternalIcon`](../../src/lib/pages/page-link-display.ts) drives the arrow-up-right affordance with a **purely relational rule**: a block renders as a subpage (no arrow) only when the canvas it lives in is the target page's current parent (`target.parentId === canvasPageId`, via [`usePageLinkCanvasPageId`](../../src/hooks/use-page-link-canvas-page-id.ts)); everywhere else it shows the arrow (a link). This makes moves auto-correct with no block writes — after a page moves, its old-parent link grows the arrow and its new-parent link drops it. `props.variant` is only a fallback for SSR / before the target or canvas id resolves (`child` → no arrow, `linked` → arrow). The canvas slash menu receives the merged page list from the page canvas editor so row components do not subscribe independently. Renaming a page updates sidebar links, slash menu entries, and every `pageLink` block automatically. Missing pages render as “Missing page”.
...
| Page menu (right) | [`PageHeaderMenu`](../../src/components/pages/page-header-menu.tsx) (single ⋯ trigger): copy link, duplicate, delete, move, font/text-size settings, add/change [cover](#page-cover), activity panel, and conditional dev/sync actions |
...
| Settings | **Font** (Default / Serif / Mono), **Text size** (Auto / Small / Default / Large), and **Full width** toggle (desktop only) — persisted on [`localPageSchema`](../../src/lib/schemas/local-page.ts) (`font`, `textScale`, `fullWidth`) and shipped [`pageSchema`](../../src/lib/schemas/page.ts); typography via [`pageContentTypographyProps`](../../src/lib/pages/page-content-typography.ts) (emits `data-page-text-scale`, which sets the `--page-text-scale` multiplier each block's `font-size: calc(<rem> * var(--page-text-scale))` reads — see `styles.css`); layout via [`pageContentColumnClassName`](../../src/lib/pages/page-content-layout.ts) |
| Cover | **Add cover** / **Change cover** opens [`PageCoverDialog`](../../src/components/pages/page-cover-menu.tsx) (Unsplash search / paste link / upload, plus an inline remove for an existing cover; reposition lives on the cover itself) — see [Page cover](#page-cover) |
...
| `textScale` | `small` \| `default` \| `large` | omit field → inherit the site-wide default ([site appearance](./site-settings.md)) |
...
On narrow viewports a cover switches the [`PageHeader`](../../src/components/pages/page-header.tsx) slot to its sticky variant (`pageCanvasMobileHeaderSlotStickyClassName`) so the bar pins to the top while the cover and page content scroll up under it; cover-less pages keep the legacy scroll-away header. The pinned bar has a **solid, opaque** background (`bg-background`, no translucency or backdrop blur — content does not show through). It also carries `pt-[env(safe-area-inset-top)]` so, with `viewport-fit=cover` (set in [`__root.tsx`](../../src/routes/__root.tsx)) letting the scroll region reach the physical top of the webview, the pinned bar tucks **behind the iOS Safari address bar / notch** (or a standalone PWA status bar, or a landscape notch): the inset grows the solid background up into the system chrome and pads the breadcrumb clear of it. In ordinary Safari browsing the inset is `0`, so it is a no-op there. The scroll region itself binds height through the full flex chain — `PageWorkspace`'s `canvasContent` wrapper is `flex flex-col` so the editor's `flex-1` scroll root is capped to the viewport and scrolls internally; without that the cover's extra height pushed content past the viewport with no way to scroll. See [Workspace layout](#workspace-layout).
