...
[`PageWorkspace`](../../src/components/pages/page-workspace.tsx) renders a resizable page sidebar ([`PageSidebar`](../../src/components/pages/page-sidebar.tsx) inside [`PageSidebarChromeProvider`](../../src/components/pages/page-sidebar-chrome.tsx)) with [`PageList`](../../src/components/pages/page-list.tsx) (`PageListContent` with `interactive: false` renders a static shell until hydration — expanded tree from the SSR cookie prefs plus active-page ancestors — then flips interactive with drag state). The shell uses shadcn [`SidebarGroup`](../../src/components/ui/sidebar.tsx) / [`SidebarMenu`](../../src/components/ui/sidebar.tsx) / [`SidebarMenuButton`](../../src/components/ui/sidebar.tsx) (sidebar accent tokens) and [`Collapsible`](../../src/components/ui/collapsible.tsx) + [`SidebarMenuSub`](../../src/components/ui/sidebar.tsx) for nested pages (rail stripped; indent via `pageListRowPaddingLeft(depth)`). Desktop uses [`ResizablePanelGroup`](../../src/components/ui/resizable.tsx) (`react-resizable-panels`, pointer resize disabled) for layout only; [`PageSidebarRail`](../../src/components/pages/page-sidebar-rail.tsx) on the main panel edge **drags** to resize between **12rem** (min) and **24rem** (max) via imperative `panel.resize()` without collapsing; width persists in `site-page-sidebar-width` ([`page-sidebar-layout-cookie.ts`](../../src/lib/pages/page-sidebar-layout-cookie.ts)). **Click** the rail (not after a drag) or the header pin control to collapse; **Cmd/Ctrl+B** toggles pin state ([`page-sidebar-pin-cookie.ts`](../../src/lib/pages/page-sidebar-pin-cookie.ts)). When collapsed, the sidebar is unmounted (zero layout width); hovering the left edge opens a floating panel over the canvas with a subtle scrim ([`PageSidebarHoverReveal`](../../src/components/pages/page-sidebar-hover-reveal.tsx)), which closes on pointer-leave; re-pin from the overlay pin button or **Cmd/Ctrl+B**. On viewports under `768px`, the [`SidebarTrigger`](../../src/components/ui/sidebar.tsx) in [`PageHeader`](../../src/components/pages/page-header.tsx) opens the page list in a Sheet (see [Workspace layout](#workspace-layout)). Expand state is hydrated from and persisted to the `site-page-list-expanded` cookie on each `expandedIds` change (see [page-commands — Page list](../reference/page-commands.md#page-list)). It shows a merged tree — shipped pages from `content/pages/**/*.json` (bundled at build via [`page-store.server.ts`](../../src/lib/content/page-store.server.ts), exposed by `listPages`) plus user-created pages in `localPagesCollection`. Every row is a `SidebarMenuItem` + `SidebarMenuButton`; nested children render inside `SidebarMenuSub` with the sub-rail stripped (`mx-0 w-full translate-x-0 border-0 px-0`) so rows stay full-width — there are no `SidebarMenuSubItem`/`SidebarMenuSubButton` rows. The branch containing the active page auto-expands on navigation. Edited server pages use the local title/slug/parentId/`sidebarOrder` when a local document exists for the same `page.id`. No separate “server” vs “your pages” sections. The root route loader prefetches the list into React Query so the sidebar stays stable across navigations.
...
Parent rows with children show a chevron on the row (`CollapsibleTrigger`) that rotates when open; expand/collapse uses `Collapsible` controlled by `expandedIds`. Row overflow uses [`SidebarMenuAction`](../../src/components/ui/sidebar.tsx) (`showOnHover`) as a three-dot [`DropdownMenu`](../../src/components/ui/dropdown-menu.tsx) trigger ([`PageListRowDropdown`](../../src/components/pages/page-list-row-menu.tsx)): Rename, Change icon, Duplicate page, Delete (disabled for home and when it would leave zero pages). **Change icon** opens the same [`PageIconPicker`](../../src/components/pages/page-icon-picker.tsx) popover anchored to the menu button (`side="right"`). Row drag uses [`usePointerClickVsDrag`](../../src/hooks/use-pointer-click-vs-drag.ts) on the draggable wrapper; chevron and overflow action use `stopPropagation` on pointer down.
