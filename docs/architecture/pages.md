...
| Page actions | **Copy link** ([`buildPageLinkUrl`](../../src/lib/pages/copy-page-link.ts)), **Duplicate page**, **Version history** (opens the full-view restore overlay), **Move to** (searchable parent picker → `page.reposition`), **Delete** (confirm dialog) — shared [`usePageActions`](../../src/hooks/use-page-actions.ts) |
...
The header menu's **Version history** action opens a full-view overlay ([`PageVersionHistoryView`](../../src/components/pages/page-version-history-view.tsx)): a version list (descriptors newest-first via [`usePageSnapshots`](../../src/db/queries/use-page-snapshots.ts)) beside a **read-only preview** of the selected checkpoint. The preview reuses the canvas renderer in view mode — [`CanvasBlocksReadOnly`](../../src/components/canvas/page-canvas-server.tsx) `mode="view"` (read-only `View` components, no `contentEditable`/gutters) wrapped in the snapshot's [`pageContentTypographyProps`](../../src/lib/pages/page-content-typography.ts), with the selected payload loaded on demand by [`usePageSnapshotContent`](../../src/db/queries/use-page-snapshot-content.ts). On narrow viewports the list and preview stack (list → tap → preview with a back control). **Restore this version** (confirm dialog) calls [`restorePageSnapshot`](../../src/lib/pages/restore-page-snapshot.ts).
