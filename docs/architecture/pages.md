...
An optional full-bleed **cover image** above the page title. The source reuses the media block encoding (`source: "url" | "asset"`): uploads become content-addressed IndexedDB assets ([`putAsset`](../../src/db/assets/asset-store.ts)); pasted links and **Unsplash** photos are stored as hotlinks (`source: "url"`) and never re-hosted. Unsplash search is proxied through Nitro ([`/api/unsplash/search`](../../routes/api/unsplash/search.get.ts), [`/api/unsplash/download`](../../routes/api/unsplash/download.post.ts), registered in [`vite.config.ts`](../../vite.config.ts)) so the `UNSPLASH_ACCESS_KEY` stays server-side; selecting a photo triggers the required download ping and stores photographer `credit` for the attribution chip ([`unsplash.ts`](../../src/lib/media/unsplash.ts)).

[`PageCover`](../../src/components/pages/page-cover.tsx) renders inside the canvas scroll region as `coverSlot` (above `headerSlot`/`titleSlot`; full-bleed via negative margins in [`page-title-layout.ts`](../../src/lib/pages/page-title-layout.ts), `object-position` driven by `focalY`). On narrow viewports a cover switches the [`PageHeader`](../../src/components/pages/page-header.tsx) slot to its sticky + `backdrop-blur` frosted variant (`pageCanvasMobileHeaderSlotStickyClassName`) so the cover scrolls up and reads as glass behind the pinned bar; cover-less pages keep the legacy scroll-away header. See [Workspace layout](#workspace-layout).
