...
- **Cross-tab sync:** TanStack DB `storage` events on page metadata and block shard keys; the canvas reads blocks from `useLiveQuery` (`usePageBlocks`). Typing writes through the transaction path on each keystroke — there is no separate draft overlay. When metadata sync changes a shipped page slug, passive tabs update the address bar via `useSyncPageUrl` (`history.replaceState` on `/$` only — user `/p/…` routes are skipped). The active tab updates the URL on title **blur** (`persistPageMetadata` with `syncUrl: true`) or, after `page.reposition` / database rehost, via TanStack Router navigate when the open path sits under the previous slug ([`resolveSlugPrefixRedirect`](../../src/lib/pages/resolve-slug-prefix-redirect.ts)).
...
Settings **Backup** ([`BackupPanel`](../../src/components/settings/panels/backup-panel.tsx)) exports or imports the local workspace as a versioned `.zip` archive (`WORKSPACE_ARCHIVE_APP` = `personal-site`, `WORKSPACE_ARCHIVE_VERSION` = 1 in [`workspace-archive.ts`](../../src/lib/content/workspace-archive.ts)). Layout: `manifest.json`, `pages/{pageId}.json` (full `pageSchema` documents — including optional authored `createdAt`/`updatedAt` from [`collectWorkspacePages`](../../src/lib/content/collect-workspace-pages.ts), resolved the same way as [pages — Page stats](./pages.md#page-stats) — with block shards inlined at export), and `media/{assetId}.{ext}` for referenced IndexedDB blobs. Zipping/unzipping uses `fflate` (async worker).
...
An open stale page shows a banner strip ([`PageStaleBanner`](../../src/components/pages/page-stale-banner.tsx), mounted by [`page-workspace.tsx`](../../src/components/pages/page-workspace.tsx)) — "This page changed on the site since you edited it" — with four resolutions:

| Action | Helper | Effect |
|--------|--------|--------|
| Merge site changes (primary) | [`mergeStalePageFromServer`](../../src/lib/pages/merge-stale-page-from-server.ts) | Three-way block merge of shipped changes into the local overlay (below); toasts a summary |
...
[`mergePageBlocks`](../../src/lib/pages/merge-page-blocks.ts) (pure, unit-tested) resolves `base` (the stored page baseline) vs `local` vs `remote` per block id — blocks are id-keyed, so no text diffing. A side unchanged from base yields to the other; identical results merge trivially; divergent changes (including edit-vs-delete in either direction) resolve to the **local side** and are reported as conflicts — a merge never silently loses a local edit. `parentId`/`indent` live in block content, so moves count as edits. Ordering: whichever side still matches the base sequence yields the ordering spine to the other (local wins when both reordered); blocks missing from the spine splice in after their nearest surviving predecessor from their own side's sequence.

[`mergeStalePageFromServer`](../../src/lib/pages/merge-stale-page-from-server.ts) orchestrates: guard the overlay exists → read the baseline ([`page-baseline-store.ts`](../../src/db/snapshots/page-baseline-store.ts); a missing baseline — pre-baseline-store overlay — falls back to the coarse resolutions) → merge → **escape hatches first** (a *pinned* forced version-history checkpoint + a session undo entry, so a bad merge is one restore or Mod+Z away) → `applyPageBlockDiff` (blocks + `blockOrder` atomically) → `keepLocalPageVersion` fast-forwards the baselines. Shipped **metadata** changes are not merged (the baseline stores block content only) — local metadata keeps rendering and the fast-forward acknowledges them.

**Pinned checkpoints:** the debounced post-merge capture lands in the same 10-minute bucket as the forced pre-merge checkpoint, and both the capture coalescing ([`resolve-snapshot-capture-action.ts`](../../src/lib/pages/resolve-snapshot-capture-action.ts)) and tiered thinning ([`thin-page-snapshots.ts`](../../src/lib/pages/thin-page-snapshots.ts)) would otherwise reclaim it immediately. Forced captures set `pinned` on the descriptor: same-bucket captures create a new checkpoint instead of updating over it, and thinning never window-collapses it (the hard 40-per-page cap still applies, so it ages out).

## Editor-mode warm cache

Once a visitor has any local workspace data, [`WarmShippedPagesCacheEffect`](../../src/components/pages/warm-shipped-pages-cache-effect.tsx) (mounted in `AppProviders`) idle-fetches [`loadAllPages`](../../src/lib/content/load-all-pages.ts) once per session and seeds every per-slug React Query cache (`["pages","by-slug",…]`; home under the literal `"home"` slug the index route uses). Loaders read with `staleTime: Infinity`, so all subsequent shipped-page navigation is served from cache — no per-page round trips for editors. Clean first-time visitors never trigger it, and a new deploy still invalidates everything via the catalog-revision effect.

## Author dev mode

In `import.meta.env.DEV`, author can save working copy directly to JSON — see [author-dev-mode](./author-dev-mode.md). [`saveAllLocalPages`](../../src/lib/content/save-all-pages.ts) writes every dirty page first, then runs `beforeClearLocal(savedPages)`, then clears local overlays **only** for successfully saved pages. The footer hook publishes each document via [`publishSavedPageToClient`](../../src/lib/content/publish-saved-page-to-client.ts) (by-slug + list React Query caches) and does **not** await `router.invalidate()` — `/` and `/$` subscribe to `pageBySlugQueryOptions` with `useQuery`, so `setQueryData` refreshes `serverPage` before the local overlay drops and the open canvas never paints pre-save shipped blocks. After clear, shipped JSON is canonical again.
