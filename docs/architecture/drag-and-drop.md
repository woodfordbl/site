...
The passive slots the canvas scroll region renders above the row list — the optional [page cover](./pages.md#page-cover) (`coverSlot`) and the in-scroll breadcrumb header (`headerSlot`, wrapped with the title/block column in a `min-w-0` block so sticky positioning works) — sit outside the `DndSurface` and are never drop targets; canvas drag math only sees `[data-canvas-row]` elements. The cover (rendered full-bleed above the content column) runs its own self-contained vertical **reposition drag** (entered from its toolbar/long-press menu; document `pointermove`/`pointerup` + rAF, panning `focalY`), reusing the low-level pattern from [`useMediaResize`](../../src/components/blocks/types/media/use-media-resize.ts) rather than the `DndSurface` channel.
...
| [`drag-image.ts`](../../src/lib/dnd/drag-image.ts) | `setEmptyDragImage`, `setClonedDragImage` | Hide native chip or clone row content (syncs `[data-canvas-field]` values); resolves the cursor hotspot — see [Preview anchoring](#preview-anchoring) |
...
Both preview paths put the pointer on the exact point the drag started from, so the ghost tracks the cursor instead of drifting.

**`native-clone` only works for a connected node whose descendants stay inside its box.** Chromium does not rasterize an element drag image from that element's border box: `DraggedNodeImageBuilder` sizes and positions the raster from `AbsoluteBoundingBoxRectIncludingDescendants()` — the union of every *layered* (positioned / clipping / transformed) descendant's border box, which `overflow: hidden`, `contain: paint`, and clipping ancestors do **not** shrink — and then reads the `setDragImage(x, y)` hotspot from that union's origin. A descendant that bleeds past the node's top-left shifts the ghost by a constant offset that no hotspot value can cancel. This bit the database block: its grid scrollport carries a negative inline `marginLeft` (the measured select-lane + panel bleed, often 300px+), so the ghost painted that far to the right of the cursor. Any future `native-clone` source inherits this constraint.

- With a compliant node, the hotspot is `clientX/Y − rect.left/top` — negative when the grab handle sits in the gutter outside the row content, which draws the ghost over the row it came from.
- Previews that cannot satisfy it use the **`overlay`** strategy instead ([`setEmptyDragImage`](../../src/lib/dnd/drag-image.ts) + a portaled React preview), which positions the preview itself and is immune to the raster-origin problem. Canvas **database** and **table** rows both take this path on *every* pointer type; other canvas rows use it on touch only.
- Overlay previews resolve their grab offset with [`resolveDragPreviewOffset`](../../src/lib/dnd/drag-image.ts), the same helper `setClonedDragImage` uses for native-clone hotspots. A detached preview (the database card) reports an `origin` — the viewport point mapping to the preview's top-left, i.e. the live `[data-database-block]` rect pushed out by the card's own `p-2` — because its measured rect is only its off-screen staging position. Live clones anchor on their own rect. Either way the offset clamps to the preview's box, since a card can be shorter than what it mirrors (the database card caps its height at 320px).
- Offsets are captured once at drag start; pointer and offsets are both viewport-space (`clientX/Y`), so mid-drag scrolling needs no correction.
...
| Canvas | [`PageCanvasEditor`](../../src/components/canvas/page-canvas-editor.tsx) | `data-canvas-row-id` | `application/x-canvas-row-id` | `native-clone` of `[data-canvas-row-content]`; **database blocks use `overlay`** — a full-block opaque card ([`resolveCanvasRowDragPreviewSource`](../../src/lib/dnd/canvas-row-drag-image.ts): title + chips + grid, `flattenDatabaseGridClone` only on the virtualized grid) rendered by [`CanvasRowDragPreview`](../../src/components/dnd/canvas-row-drag-preview.tsx), because its bleeding grid breaks the native raster origin (see [Preview anchoring](#preview-anchoring)) | `CanvasDropZone` div | [`resolveDropTargetFromPointer`](../../src/lib/canvas/resolve-drop-target.ts) |
...
- Sidebar: [`PageListItem`](../../src/components/pages/page-list-item.tsx) uses `useDropTarget` for sibling lines and nest highlight. Hosted-database rows ([`page-list-database-rows.tsx`](../../src/components/pages/page-list-database-rows.tsx) / [`DatabaseSidebarRow`](../../src/components/pages/database-sidebar-row.tsx)) are drag sources on the same page-list surface with a prefixed source id (`database:{id}` via [`database-list-drag.ts`](../../src/lib/databases/database-list-drag.ts)); they are not page-row drop targets (`data-page-list-row-id` stays pages-only). Dropping a database onto a page's nest band rehosts it ([`moveDatabaseHost`](../../src/lib/databases/move-database-host.ts) — strip non-owned host blocks, append on the new host, reparent the hub); when the open tab is on that hub or a row under it, [`PageList`](../../src/components/pages/page-list.tsx) router-navigates via [`resolveSlugPrefixRedirect`](../../src/lib/pages/resolve-slug-prefix-redirect.ts). The workspace **Databases** section stays outside the page-list surface and is not draggable. Row ⋯ / right-click menus ([`PageRowMenuContent`](../../src/components/pages/page-row-menu-content.tsx), including **Copy link**) stop propagation on their triggers so opening an action never starts a page-list drag.
