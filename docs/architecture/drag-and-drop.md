...
| Canvas | [`PageCanvasEditor`](../../src/components/canvas/page-canvas-editor.tsx) | `data-canvas-row-id` | `application/x-canvas-row-id` | `native-clone` of `[data-canvas-row-content]` (database blocks: full-block opaque card via [`resolveCanvasRowDragPreviewNode`](../../src/lib/dnd/canvas-row-drag-image.ts) — title + chips + grid; `flattenDatabaseGridClone` only on the virtualized grid) | `CanvasDropZone` div | [`resolveDropTargetFromPointer`](../../src/lib/canvas/resolve-drop-target.ts) |
...
Native file drags (drag type `Files`) never enter the internal DnD channel:
the sidebar nav and the canvas drop zone branch on `dragHasFiles` BEFORE the
internal handlers. `.md` files import as new pages (sidebar) or insert parsed
blocks at the drop position (canvas); image/video files insert media blocks.
See [markdown-content-format — Clipboard & file drops](./markdown-content-format.md#clipboard--file-drops).
