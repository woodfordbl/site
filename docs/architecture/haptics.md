...
| [`page-sidebar-swipe-reveal.tsx`](../../src/components/pages/page-sidebar-swipe-reveal.tsx) | `selection` | Mobile sidebar swipe crossing the open/closed snap line — from any of the three gesture surfaces (left-edge strip, content scrim, or the sidebar panel/page list itself; all share one drag) — fired mid-drag in `pointermove`, not on release (iOS produces no feedback from the pointerup that ends a captured drag), plus the backdrop tap that closes. Vertical swipes on the panel scroll the page list and never tick |
