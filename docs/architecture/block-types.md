...
| `container` | list, checklist, columns, column, table, tableRow | list → `["text"]`; checklist → `["checklistItem"]`; columns → `["column"]`; column → `*` (full canvas rows, no lift-out); table → `["tableRow"]`; tableRow → `["tableCell"]`. `columns` renders via [`ColumnsView`](../../src/components/blocks/types/columns/columns-view.tsx); column children use [`BlockTreeNode`](../../src/components/canvas/block-tree-node.tsx). `table` renders via [`TableView`](../../src/components/blocks/types/table/table-view.tsx) (cells inline — no nested gutters). Resizable widths: `column.props.width` (columns) and `table.props.columnWidths` (table). Slash: **2/3/4 columns** via `columns.create`; **Table** via `table.create` (default 3×3). See [table-blocks](./table-blocks.md). |
