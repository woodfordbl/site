...
| Development (labeled **Local changes** in production) | [`DevelopmentPanel`](../../src/components/settings/panels/development-panel.tsx) | Save all (dev-only), reset, refresh — shown only when [`usePageCanvasFooterActions`](../../src/hooks/use-page-canvas-footer-actions.ts) reports `visible`; the label is compile-time switched in [`site-settings-sections.ts`](../../src/components/settings/site-settings-sections.ts) since visitors see this section too |
