...
| `#` | All canvases | Open [`FormulaEditorPanel`](../../src/components/database/formula-editor-panel.tsx) (`layout="popover"`) under the caret; **Save** replaces `#…` with an inline formula token (same mark as slash / typing `{{`). On database row and template pages, Properties lists the database's columns layered over the base page fields (Title / Created at / Updated at) via [`InlineFormulaPageProvider`](../../src/components/editor/inline-formula-page.tsx) + [`page-formula-fields.ts`](../../src/lib/databases/page-formula-fields.ts) |

Hovering a token in an editable field shows [`InlineFormulaTokenTooltip`](../../src/components/canvas/inline-formula-tooltip.tsx) — **Edit formula** over the expression behind the value on screen — delegated from one per-canvas instance for the same reason the popover is, and dismissed by any press, resize, or scroll. It replaces the native `title` the token used to carry ([`rich-text-dom.ts`](../../src/lib/editor/rich-text-dom.ts) no longer sets one, in the builder or the serializer): a browser tooltip arrives late, in the OS's chrome, and cannot say what the click does. Read-only tokens have nothing to edit, so they get no hint.

Typing `{{` still inserts an empty formula token and opens [`InlineFormulaPopover`](../../src/components/canvas/inline-formula-popover.tsx) (legacy mid-sentence path). Markdown headings still use `#`/`##`/… + **Space** when the field value is exactly those characters ([`editable-surface.tsx`](../../src/components/editor/editable-surface.tsx)); Escape on an open `#` formula session leaves plain `#` so that path still works.
