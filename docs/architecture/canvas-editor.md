...
| `#` | All canvases | Open [`FormulaEditorPanel`](../../src/components/database/formula-editor-panel.tsx) (`layout="popover"`) under the caret; **Save** replaces `#…` with an inline formula token (same mark as slash / typing `{{`) |
...
**Inline page links** are the same `link` mark with an optional `pageId` (plus `href` for the share URL). They render as a compact [`InlinePageLink`](../../src/components/editor/inline-page-link.tsx) (page icon + underlined title + upright arrow — same chrome as a `pageLink` block) and navigate in-app; they skip OG preview preload/hover. Edit mode keeps `contenteditable=false` icon/arrow hosts filled via portals. Sizing is inherited, not fixed: the title carries no font size of its own, and the icon/arrow are sized in `em` (`inlinePageLinkIconClassName` / `inlinePageLinkArrowClassName`) so the run reads at the size of the text it sits in — a fixed-px icon looked undersized in a heading. The anchor is `align-middle` because an `inline-flex` box otherwise takes its baseline from its first item — the icon slot, empty until the portal fills it — and rides above the line.
