...
The editor scroll region ([`page-canvas-editor.tsx`](../../src/components/canvas/page-canvas-editor.tsx), `[data-scroll-restoration-id="page-canvas-scroll"]`) renders three passive slots above the row list, in order: `coverSlot` (optional full-bleed [page cover](./pages.md#page-cover)), `headerSlot` (the mobile breadcrumb bar), and `titleSlot`. They are inert content, not editable rows or drop targets, so the read-only views ([`page-canvas-server.tsx`](../../src/components/canvas/page-canvas-server.tsx)) thread the same slots for SSR/no-layout-shift parity.
