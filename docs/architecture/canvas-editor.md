...
**Inline page links** are the same `link` mark with an optional `pageId` (plus `href` for the share URL). They render as a compact [`InlinePageLink`](../../src/components/editor/inline-page-link.tsx) — page icon + live title, one rule under both, no arrow — and navigate in-app; they skip OG preview preload/hover. Edit mode keeps the `contenteditable=false` icon host filled via a portal. Sizing and colour are inherited, not fixed: the title carries no font size of its own, the icon is sized in `em` (`inlinePageLinkIconClassName`) so the run reads at the size of the text it sits in, and the run takes the prose colour — hover moves the rule, not the text.
