...
**Inline page links** are the same `link` mark with an optional `pageId` (plus `href` for the share URL). They render as a compact [`InlinePageLink`](../../src/components/editor/inline-page-link.tsx) (page icon + underlined title + upright arrow — same chrome as a `pageLink` block) and navigate in-app; they skip OG preview preload/hover. Edit mode keeps `contenteditable=false` icon/arrow hosts filled via portals.
