...
The editor scroll region ([`page-canvas-editor.tsx`](../../src/components/canvas/page-canvas-editor.tsx), `[data-scroll-restoration-id="page-canvas-scroll"]`) renders passive slots around the row list: `coverSlot` (optional full-bleed [page cover](./pages.md#page-cover)) and `headerSlot` (the breadcrumb bar, when it lives in-scroll — mobile, or desktop with a cover) are direct children of the scroll root so they bleed edge to edge, while `titleSlot` sits inside the centered content column (`pageContentColumnClassName`, unless the page is full-width). They are inert content, not editable rows or drop targets, so the read-only views ([`page-canvas-server.tsx`](../../src/components/canvas/page-canvas-server.tsx)) thread the same slots for SSR/no-layout-shift parity.
...
`PageCanvas` owns the scroll region: the page title (`titleSlot`) and block list scroll full-height inside the inset card. On desktop the [`PageHeader`](../../src/components/pages/page-header.tsx) breadcrumb is a fixed bar above the scroll region; on narrow viewports it is passed as `headerSlot` and rendered **inside** the scroll region so it scrolls away with the content (nothing pinned to the top). The scroll region also tightens its padding on narrow viewports (`pl-7 pr-4 pb-4`, narrower than the desktop `px-12 py-12`) and the gutter grip shrinks to match so blocks get more usable width. On desktop, when a page’s `fullWidth` setting is off (default), title + blocks render in a centered **708px** column via [`pageContentColumnClassName`](../../src/lib/pages/page-content-layout.ts) inside the scroll inset; [`PageContentLayoutProvider`](../../src/lib/pages/page-content-layout-context.tsx) exposes whether block types (e.g. tables) may bleed into panel padding. Mobile always uses full-bleed layout regardless of the setting. See [pages — Page settings](./pages.md#page-settings). The author toolbar ([`PageCanvasFooter`](../../src/components/canvas/page-canvas-footer.tsx)) is rendered at **workspace level** ([`page-workspace.tsx`](../../src/components/pages/page-workspace.tsx)), decoupled from the lazily-loaded editor chunk so it is always present — not an in-flow footer. On desktop it sits in a strip below the inset card on the `bg-sidebar` surface; on narrow viewports that host is hidden and the same actions surface via [`PageCanvasActionsDrawer`](../../src/components/canvas/page-canvas-actions-drawer.tsx) from a trigger in the scrolling `PageHeader`. All actions are global (no editor state):
