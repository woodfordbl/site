...
The editor scroll region ([`page-canvas-editor.tsx`](../../src/components/canvas/page-canvas-editor.tsx), `[data-scroll-restoration-id="page-canvas-scroll"]`) renders passive slots around the row list: `coverSlot` (optional full-bleed [page cover](./pages.md#page-cover)) is a direct child of the scroll root so it bleeds edge to edge, while `headerSlot` (the mobile breadcrumb bar) and `titleSlot` sit inside the centered content column (`pageContentColumnClassName`, unless the page is full-width). They are inert content, not editable rows or drop targets, so the read-only views ([`page-canvas-server.tsx`](../../src/components/canvas/page-canvas-server.tsx)) thread the same slots for SSR/no-layout-shift parity.
