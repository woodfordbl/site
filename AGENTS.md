...
- **Sonner toasts:** Fully design-system styled via `sonner.tsx` + `styles.css` `.cn-toast` (no `richColors`); popover surface tokens; Tabler type icons `size-4 stroke-[1.5px]`; layout — type icon top-left aligned to first text line, title/description and ghost `xs` action/cancel row below aligned with the text column (not the icon), `gap-2`; close is ghost `icon-sm` `rounded-full` top-right inset; non-front stacked toasts hide action buttons; preview all variants in `/dev` `ToastsSection`.
